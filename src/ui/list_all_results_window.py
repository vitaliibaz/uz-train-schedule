	...
	def prepare_window_all_results(self, args):
		...
		for i in range(len(routes)):
			...
			if route.first_train.number_train != route.second_train.number_train:
				data[i] = {'text': '{0} - {1} ({2} год.)\nз пересадкою'.decode('utf-8').format(ftdt, stat, tt), 'route': route}
			else:
				data[i] = {'text': '{0} - {1} ({2} год.)\nбез пересадки'.decode('utf-8').format(ftdt, stat, tt), 'route': route}

		dict_adapter = DictAdapter(data=data,
								   args_converter=self.result_converter,
								   template='CustomListItem')

		button_back = Button(pos_hint={"right":1, "top":1}, text="Back".decode('utf-8'), multiline=False, font_size=30, color = [1,1,1,1], background_color=[1,0,0,1], on_release=self.back_menu)
		message_about_list_result = Label(pos_hint={"top":0.975}, text="Це результати пошуку.\nОберіть маршрут, що підходить.\nНатисніть".decode('utf-8'), multiline=True, size_hint_y=0.25, font_size=30)
