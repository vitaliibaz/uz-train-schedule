...
class ResultDetailsWindow(Screen):
	...
	def prepare_details_window(self, args):
		...
		self.ids.departure.text = u'Від: {0} - {1}'.format(details_route.first_train.departure_time, departure_station.name)
		self.ids.arrival.text = u'До:  {0} - {1}'.format(details_route.second_train.arrival_time, arrival_station.name)
		self.ids.total_time.text = u'Час в дорозі: {0} год.'.format(details_route.total_time)
		self.ids.number_train_and_route.text = u'Відправлення: поїзд {0} {1}'.format(details_route.first_train.number_train, details_route.first_train.route)
		...
		if details_route.first_train.number_train == details_route.second_train.number_train:
			self.ids.via.text = u'Без пересадки'
		else:
			self.ids.transfer.text = u'Час очікування і станція пересадки:\n'
			self.ids.via.text = u'Пересадка: поїзд {0} {1}\n'.format(details_route.second_train.number_train, route.second_train.route)
