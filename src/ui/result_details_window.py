...
class ResultDetailsWindow(Screen):
	...
	def prepare_details_window(self, args):
		...
		self.ids.departure.text = details_route.first_train.departure_station
		...
		if details_route.first_train.number_train != details_route.second_train.number_train:
			self.ids.via.text = '{0} - {1} {2}'.format(details_route.first_train.arrival_time, details_route.second_train.departure_time, details_route.second_train.departure_station)
		else:
		...
		# self.ids.number_train.text = details_route.first_train.number_train
